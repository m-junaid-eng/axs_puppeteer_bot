...
async function fetchAxsLinksFromSupabase({ limit = 50 } = {}) {
  const { data, error } = await supabase
    .from('axs_links')
    .select('url, list_event_url, created_at')
    .order('created_at', { ascending: false })
    .limit(limit);

  if (error) {
    console.error('Supabase select error:', error);
    return { entryUrl: null, allowedEventUrls: new Set() };
  }

  const rows = Array.isArray(data) ? data : [];
  const entryUrl = rows.find((r) => r?.list_event_url)?.list_event_url ?? null;
  const allowedEventUrls = new Set(
    rows
      .map((r) => canonicalizeHttpUrl(r?.url))
      .filter((u) => Boolean(u) && u.startsWith('http'))
  );

  return { entryUrl, allowedEventUrls };
}
...
function isAbortLikeError(err) {
  ...
  return name === 'AbortError' || code === 'ABORT_ERR' || msgLc.includes('net::err_aborted') || msgLc.includes('aborted');
}

async function safeGoto(page, targetUrl, options, { retries = 1 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      lastErr = err;
      if (attempt < retries && isAbortLikeError(err)) {
        await sleep(750);
        continue;
      }
      throw err;
...
async function safeScreenshot(page, options) {
  try {
    ...
    const msg = String(err?.message ?? err);
    if (isAbortLikeError(err) || msg.toLowerCase().includes('target closed')) {
      console.warn('Screenshot skipped due to transient page error:', msg);
      return false;
    }
    throw err;
  }
...
async function main() {
  const { entryUrl, allowedEventUrls } = await fetchAxsLinksFromSupabase({ limit: 50 });
  const url = getArgValue('--url') ?? entryUrl ?? DEFAULT_URL;
...
const activeProxy = PROXY_LIST.UK_1; 
...
await page.authenticate({
  ...
  try {
    ...
    await safeGoto(page, url, { waitUntil: 'domcontentloaded', timeout: 60_000 });
    await sleep(3000);
    ...
    if (await safeScreenshot(page, { path: screenshotPath, fullPage: true })) {
    ...
    if (followAllEvents) {
      ...
      if (hasAllEventsSection) {
        ...
        if (allEventsHref) {
          ...
          await safeGoto(page, allEventsHref, { waitUntil: 'domcontentloaded', timeout: 60_000 });
          await sleep(2000);
          await waitForManualCaptchaSolve(page, { interactive });
          await waitForAllEventsHeading(page, { timeoutMs: interactive ? 60_000 : 25_000 });
        } else {
      ...
      if (allowedEventUrls.size > 0) {
        const before = links.length;
        links = links.filter((l) => allowedEventUrls.has(canonicalizeHttpUrl(l?.href)));
        console.log(`Filtered event links by DB urls: ${links.length}/${before} remain.`);
      } else {
        console.log('No event URLs found in Supabase; nothing to visit.');
        return;
      }
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          if (ticketsHref) {
            ...
            try {
              ...
              if (!clickOutcome?.navigated) {
                await safeGoto(ticketsPage, ticketsHref, {
              ...
              await safeScreenshot(ticketsPage, { path: ticketsShotPath, fullPage: true });
          ...
          await safeScreenshot(eventPage, { path: shotPath, fullPage: true });
