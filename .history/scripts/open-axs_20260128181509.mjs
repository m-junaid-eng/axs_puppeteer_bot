...
function isAxsPriceApiUrl(rawUrl) {
	try {
		const u = new URL(String(rawUrl));
		if (!u.hostname.endsWith('axs.com')) return false;
		if (!u.pathname.includes('/veritix/inventory/v4/')) return false;
		if (!u.pathname.endsWith('/price')) return false;
		return true;
	} catch {
		return false;
	}
...
function flattenPriceResponseToRows(priceJson, { sourceUrl = '', capturedAt = '', eventId = '' } = {}) {
	const rows = [];
	const currency = priceJson?.currency ?? '';
	const offerPrices = Array.isArray(priceJson?.offerPrices) ? priceJson.offerPrices : [];
	for (const offer of offerPrices) {
		...
		for (const zone of zonePrices) {
			const priceLevels = Array.isArray(zone?.priceLevels) ? zone.priceLevels : [];
			for (const level of priceLevels) {
				...
				for (const p of prices) {
					...
					rows.push({
						...
						offerLookupId: offer?.offerLookupId ?? '',
						offerName: offer?.offerName ?? '',
						offerType: offer?.offerType ?? '',
						zoneID: zone?.zoneID ?? '',
						productID: zone?.productID ?? '',
						zoneEventID: zone?.eventID ?? '',
						priceLevelLabel: level?.label ?? '',
						priceLevelID: level?.priceLevelID ?? '',
						availabilityAmount: level?.availability?.amount ?? '',
						priceTypeID: p?.priceTypeID ?? '',
						baseCents: base,
						baseDollars: typeof base === 'number' ? base / 100 : ''
					});
				}
			}
		}
...
async function waitForPriceApiResponse(page, { timeoutMs = 45_000 } = {}) {
	// Waits for the first JSON response from the AXS ".../price" inventory API.
	return await new Promise((resolve) => {
		...
		const onResponse = async (res) => {
			try {
				const url = res.url();
				if (!isAxsPriceApiUrl(url)) return;
				...
				for (let i = 0; i < toVisit.length; i++) {
					...
					if (ticketsHref) {
						...
						try {
							...
							// Start listening for the prices API before the user clicks around.
							const priceWait = waitForPriceApiResponse(ticketsPage, {
								timeoutMs: interactive ? 5 * 60_000 : 45_000
							});
							...
							const priceCapture = await priceWait;
							if (priceCapture?.data) {
								const pricesJsonPath = path.join(eventsDir, `${base}-prices.json`);
								await fs.writeFile(pricesJsonPath, JSON.stringify(priceCapture.data, null, 2), 'utf8');

								const rows = flattenPriceResponseToRows(priceCapture.data, {
									sourceUrl: priceCapture.url,
									capturedAt,
									eventId
								});
								const pricesXlsxPath = path.join(eventsDir, `${base}-prices.xlsx`);
								// Reuse the XLSX writer (sheet name isn't critical per-file).
								await writeXlsx({ rows, filePath: pricesXlsxPath });
								console.log(`Saved prices JSON: ${pricesJsonPath}`);
								console.log(`Saved prices XLSX: ${pricesXlsxPath}`);
							} else {
								console.log('Did not capture a prices API response on the tickets page (yet).');
							}
