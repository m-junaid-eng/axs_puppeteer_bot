        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          if (ticketsHref) {
            ...
            try {
              ...
              // --- RE-TRIGGER RELOAD TO CATCH DATA AFTER SOLVE ---
              console.log('Refreshing Seat Map to capture prices...');
              await ticketsPage.reload({ waitUntil: 'networkidle2' });
              await sleep(4000); 

              const ticketsHtmlPath = path.join(eventsDir, `${eventId}-${safeFilename(details.name)}-tickets.html`);
              await saveHtml({ page: ticketsPage, filePath: ticketsHtmlPath });
              
              // --- ASSIGN DATA FOR XLSX ---
              details.prices = interceptedPrices.length > 0 ? JSON.stringify(interceptedPrices) : "N/A";

              const ticketsShotPath = path.join(eventsDir, `${eventId}-${safeFilename(details.name)}-tickets.png`);
          ...
          await eventPage.screenshot({ path: path.join(eventsDir, `${base}.png`), fullPage: true }); else {
