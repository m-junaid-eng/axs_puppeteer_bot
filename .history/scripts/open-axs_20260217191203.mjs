...
async function getEntryUrlFromSupabase() {
  const { data, error } = await supabase
    .from('axs_links')
    .select('list_event_url, created_at')
    .not('list_event_url', 'is', null)
  ...
  const url = String(row?.list_event_url ?? '').trim();
...
async function main() {
  const url = (await getEntryUrlFromSupabase()) ?? DEFAULT_URL;
