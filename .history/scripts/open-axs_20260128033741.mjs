...
async function waitForManualCaptchaSolve(page, { interactive }) {
  ...
  console.log('--- Action: Solve the Cloudflare Checkbox ---');

  // 1. Wait for the challenge iframe to actually disappear from the page
  ...
  // 2. The "Success" delay
  // After the checkbox turns into a green check, Cloudflare sends a final 
  // validation token. We MUST wait for this to finish.
  console.log('Checkmark detected! Waiting 8 seconds for background validation...');
  await sleep(8000); 

  // 3. Force a small scroll to prove "human" activity to AXS sensors
...
async function main() {
  ...
  headless: false,
  userDataDir: userDataDir, // Use your persistent profile
  ignoreDefaultArgs: ['--enable-automation'], // REMOVES the "Chrome is being controlled" notification
  args: [
    '--disable-blink-features=AutomationControlled', // Hides window.navigator.webdriver
    '--no-sandbox',
    '--start-maximized',
    '--disable-infobars',
    '--disable-dev-shm-usage',
    '--remote-debugging-port=9222' // Standard port for real debugging
  ],
