...
async function waitForManualChallengeClear(page, { interactive, timeoutMs = 10 * 60_000 } = {}) {
	// Generic variant for non-event pages (e.g., tickets checkout) where we only
	...
	console.log('Cloudflare/CAPTCHA challenge detected (non-event page).');
	...
	while (Date.now() - started < timeoutMs) {
		...
		// Require it to stay cleared for a bit (tickets flows often redirect and can re-challenge).
		clearedStreak++;
		if (clearedStreak < 30) {
			await sleep(500);
			continue;
		...
		if (followAllEvents) {
			...
			try {
				...
				for (let i = 0; i < toVisit.length; i++) {
					...
					// Also capture the "Get Tickets" page HTML (in the SAME tab).
					...
					if (ticketsHref) {
						...
						await sleep(1500);
						await waitForManualChallengeClear(eventPage, { interactive });
