...
async function findGetTicketsUrl(page) {
	// Prefer stable attributes that exist on AXS EDP.
	const href = await page.evaluate(() => {
		const byTestId = document.querySelector('a[data-testid="LinkButton"][href]');
		if (byTestId) {
			const aria = (byTestId.getAttribute('aria-label') || '').toLowerCase();
			const text = (byTestId.textContent || '').trim().toLowerCase();
			if (aria.includes('get tickets') || text.includes('get tickets')) return byTestId.href;
		}
		...
		const candidate = anchors.find((a) => {
			...
			if (!(aria.includes('get tickets') || text.includes('get tickets'))) return false;
			const href = (a.href || '').toLowerCase();
			return href.includes('tix.axs.com') || href.includes('axs.com');
		});
...
async function waitForOfferPricesJson(page, { timeoutMs }) {
	/** @type {any | null} */
	let found = null;
	let done = false;
	...
	const onResponse = async (res) => {
		...
		try {
			...
			const url = res.url();
			// Light heuristic: most useful responses contain offerPrices.
			if (!url) return;

			const data = await res.json().catch(() => null);
			if (!data || typeof data !== 'object') return;
			if (!Array.isArray(data.offerPrices)) return;
	...
	try {
		...
		while (!done && Date.now() - started < timeoutMs) {
			...
			if (await isLikelyCaptchaPage(page)) {
				// Let the existing manual solve flow handle it.
				await sleep(500);
				continue;
			}
			await sleep(250);
...
function flattenOfferPrices(offerJson, { eventId, eventUrl, ticketsUrl }) {
	const currency = String(offerJson?.currency || '');
	const out = [];
	const offers = Array.isArray(offerJson?.offerPrices) ? offerJson.offerPrices : [];

	for (const offer of offers) {
		const offerID = String(offer?.offerID ?? '');
		const offerGroupID = String(offer?.offerGroupID ?? '');
		const offerLookupId = String(offer?.offerLookupId ?? '');
		const offerName = String(offer?.offerName ?? '');

		const zonePrices = Array.isArray(offer?.zonePrices) ? offer.zonePrices : [];
		for (const zp of zonePrices) {
			const zoneID = zp?.zoneID ?? '';
			const productID = zp?.productID ?? '';
			const apiEventID = zp?.eventID ?? '';

			const priceLevels = Array.isArray(zp?.priceLevels) ? zp.priceLevels : [];
			for (const pl of priceLevels) {
				const priceLevelID = String(pl?.priceLevelID ?? '');
				const priceLevelLabel = String(pl?.label ?? '');
				const availabilityAmount = pl?.availability?.amount ?? '';
				const prices = Array.isArray(pl?.prices) ? pl.prices : [];
				...
				for (const p of prices) {
					const baseCents = typeof p?.base === 'number' ? p.base : '';
					const baseUSD = typeof p?.base === 'number' ? (p.base / 100).toFixed(2) : '';
					const priceTypeID = String(p?.priceTypeID ?? '');
					const comps = Array.isArray(p?.priceComponents) ? p.priceComponents : [];
					const components = comps
						.map((c) => `${String(c?.name ?? '')}:${String(c?.amount ?? '')}`)
						.filter(Boolean)
						.join(' | ');
...
async function writeXlsxWithSheets({ sheets, filePath }) {
	const wb = XLSX.utils.book_new();
...
async function main() {
	...
	const includePrices = followAllEvents && !hasFlag('--no-prices');
		...
		if (followAllEvents) {
			...
			try {
				...
				for (let i = 0; i < toVisit.length; i++) {
					...
					if (includePrices) {
						const ticketsUrl = await findGetTicketsUrl(eventPage);
						if (!ticketsUrl) {
							console.log(`No "Get Tickets" link found for event ${eventId}.`);
						} else {
							console.log(`Opening tickets page for event ${eventId}...`);
							const ticketsPage = await browser.newPage();
							try {
								if (userAgent && !noUaOverride) {
								...
								const waitJsonPromise = waitForOfferPricesJson(ticketsPage, {
									timeoutMs: interactive ? 180_000 : 45_000
								});
								await ticketsPage.goto(ticketsUrl, {
									waitUntil: 'domcontentloaded',
									timeout: 60_000,
									referer: href
								});
								await sleep(1500);
								await waitForManualCaptchaSolve(ticketsPage, { interactive });
								if (interactive) {
									await promptEnter(
										'On tickets page. If needed, click to load prices, then press Enter to capture... ',
										{ skipIfNoTty: true }
									);
								}

								const offerJson = await waitJsonPromise;
								if (!offerJson) {
									console.log(`No offerPrices JSON captured for event ${eventId}.`);
								} else {
									const pricesDir = path.join(artifactsDir, 'prices');
									await fs.mkdir(pricesDir, { recursive: true });
									const rawPath = path.join(pricesDir, `${eventId}.json`);
									await fs.writeFile(rawPath, JSON.stringify(offerJson, null, 2), 'utf8');
									console.log(`Saved prices JSON: ${rawPath}`);

									const flat = flattenOfferPrices(offerJson, {
										eventId,
										eventUrl: details.url || href,
										ticketsUrl
									});
									priceRows.push(...flat);
								}
			...
			if (exportXlsx) {
				...
				if (includePrices && priceRows.length > 0) {
					await writeXlsxWithSheets({
						sheets: [
							{ name: 'events', rows: scraped },
							{ name: 'prices', rows: priceRows }
						],
						filePath: xlsxPath
					});
				} else {
