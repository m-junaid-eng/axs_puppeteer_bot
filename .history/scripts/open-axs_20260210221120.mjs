...
// 1. Updated Credentials (Added 'user-' prefix which is mandatory)
const PROXY_USER = 'user-watchdog_cdL8M'; 
const PROXY_PASS = 'Watchdog_4990';
const UK_PORT = '8006'; 
const PROXY_SERVER = `isp.oxylabs.io:${UK_PORT}`; // Removed http:// from here for the arg
...
const { browser, page } = await connect({
    ...
    args: [
        ...
        '--disable-http2', // AXS often resets H2 connections over proxies
...
// 3. Force a "Waiting Period" before navigation
// This allows the proxy tunnel to stabilize
console.log(`Stabilizing Oxylabs tunnel on port ${UK_PORT}...`);
await sleep(2000); 

console.log(`Connected via Oxylabs UK Proxy`);
  try {
