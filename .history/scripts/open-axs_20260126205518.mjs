...
async function isLikelyCaptchaPage(page) {
  try {
    ...
    if (url.includes('captcha') || url.includes('challenge') || url.includes('arkoselabs')) return true;
    ...
    const hasCaptchaHints = await page.evaluate(() => {
      ...
      if (text.includes('captcha') || text.includes('verify you are') || text.includes('are you human')) return true;
...
async function waitForManualCaptchaSolve(page, { interactive }) {
  ...
  console.log('CAPTCHA detected. Solve it in the opened browser window.');
  console.log('After it finishes and the event page loads, press Enter here to continue.');
