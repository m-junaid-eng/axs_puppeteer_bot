...
async function waitForManualCaptchaSolve(page, { interactive }) {
  ...
  // Poll briefly so we reliably catch Cloudflare "Just a moment" and similar.
  ...
  console.log('Solve it in the opened browser tab, wait until the real event page loads, then press Enter here.');
  ...
  // Loop: user may press Enter too early; keep checking until challenge clears.
  while (await isLikelyCaptchaPage(page)) {
    await promptEnter('After solving, press Enter to continue... ');
    // Give the site time to redirect after solve.
    for (let i = 0; i < 60; i++) {
      if (!(await isLikelyCaptchaPage(page))) return;
      await sleep(1000);
    }
    console.log('Still looks like a challenge page. If you have not finished solving, complete it and press Enter again.');
  }
}
...
async function extractEventDetails(page, { fallbackUrl, eventId }) {
  ...
  if (eventNode) {
    ...
    // Fallbacks if JSON-LD not present.
...
async function findFirstLinkByText(page, needle) {
  ...
  const href = await page.$$eval('a[href]', (as, lowerNeedleIn) => {
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    try {
      found = await page.evaluate(() => {
        ...
        return hasAllEventsHeading || hasAllEventsLink || hasSeeEvent;
      });
    } catch (err) {
      const msg = String(err?.message || err);
      // AXS sometimes triggers a navigation/redirect while we're polling.
    ...
    // AXS often lazy-renders sections while scrolling.
    if (scrolls < 6) {
      try {
...
async function collectSeeEventLinks(page) {
  // Prefer explicit anchors; buttons that navigate are harder to generalize safely.
...
async function waitForLikelyEventPage(page, { timeoutMs }) {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    // If we're back on a challenge, stop waiting.
    ...
    const hasEventHints = await page.evaluate(() => {
      ...
      // Weak fallback: AXS pages usually have og:title.
...
async function main() {
  ...
  const postSolveWaitMs = postSolveWaitMsRaw
    ? Number(postSolveWaitMsRaw)
    : interactive
      ? 6000
      : 0;
  ...
  // User-Agent strategy:
  // - In interactive mode, default to a stable UA (no override) unless --ua or --rotate-ua is provided.
  // - Outside interactive mode, default remains rotating from uas.json.
  ...
  // Profile strategy:
  // - If --fresh-profile, always use a new profile.
  // - If --profile is given, use that directory.
  // - If interactive and neither flag is provided, use a persistent profile to reduce repeated challenges.
  let userDataDir;
  if (freshProfile) {
  ...
  try {
    ...
    if (interactive && headlessRequested) {
      console.log('Note: --interactive forces headful mode (ignoring --headless).');
    }
    ...
    if (dumpFingerprint) {
      ...
      // Write both a stable "latest" file and a timestamped history entry.
      ...
      // Append a simple text log (TSV) so you can diff runs easily.
    ...
    if (followAllEvents) {
      ...
      // The All Events section is frequently rendered asynchronously.
      await waitForAllEventsArea(page, { timeoutMs: interactive ? 30_000 : 15_000 });

      // Many AXS pages render an "All Events" section on the same page.
      ...
      if (hasAllEventsSection) {
        ...
        // Fallback: try to navigate via a link, if the site uses a separate page.
        ...
        if (allEventsHref) {
          ...
          // Some pages still contain "See Event" links without a labeled "All Events" section.
          console.log('No explicit "All Events" section/link found; trying to locate "See Event" links on the current page...');
        }
      }
      ...
      if (links.length === 0) {
        links = await collectSeeEventLinks(page);
      }
      if (links.length === 0) {
        console.log('No "See Event" links found after waiting/scrolling.');
        return;
      }

      const toVisit = links.slice(0, Number.isFinite(maxEvents) ? maxEvents : 10);
      console.log(`Found ${links.length} "See Event" links; visiting ${toVisit.length}.`);

      const eventsDir = path.join(artifactsDir, 'events');
      await fs.mkdir(eventsDir, { recursive: true });

      // Use a single event tab so manual CAPTCHA solving is manageable.
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          if (interactive && stepMode) {
            await promptEnter('Press Enter to open the next event link... ');
          }

          await eventPage.goto(href, { waitUntil: 'domcontentloaded', timeout: 60_000 });
          await sleep(1500);

          // If the site shows a CAPTCHA/challenge page, let the user solve it manually.
          await waitForManualCaptchaSolve(eventPage, { interactive });

          if (interactive && postSolveWaitMs > 0) {
            // Give time for redirect/session cookies to settle before we read the page or move on.
            await sleep(postSolveWaitMs);
          }

          // Optional: wait briefly for likely event page markers so we don't scrape mid-redirect.
          if (interactive) {
            await waitForLikelyEventPage(eventPage, { timeoutMs: 20_000 });
          }

          // If we're still stuck on a CAPTCHA page, skip scraping.
