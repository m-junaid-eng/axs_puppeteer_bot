  ...
  try {
    ...
    if (supabaseMode) {
      ...
      for (let i = 0; i < urls.length; i++) {
        const ticketsUrl = urls[i];
        console.log(`Tickets ${i + 1}/${urls.length}: ${ticketsUrl}`);
        ...
        try {
          ...
          await ticketsPage.goto(ticketsUrl, { waitUntil: 'networkidle2', timeout: 60_000 });
          await sleep(1500);
          ...
          const base = `${String(i + 1).padStart(3, '0')}-${safeFilename(ticketsUrl)}`;
