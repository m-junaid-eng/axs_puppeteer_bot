...
const activeProxy = PROXY_LIST.UK_1; // Example: use the German proxy
const { browser, page } = await connect({
    ...
    args: [
        ...
        '--lang=en-GB', // Match the UK Proxy
        '--no-sandbox'
    ],
    // ADD THIS: It forces the fingerprint to look cleaner
    customConfig: {
...
// Use Supabase data instead of scanning the page
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
          const { href, label, listingReferer: rowReferer } = toVisit[i];
          const idMatch = String(href).match(/\/events\/(\d+)/);
          const eventId = idMatch?.[1] ?? String(i + 1);

          console.log(`\n--- Processing Event ${i + 1}/${toVisit.length}: ${href} ---`);

          // --- FIX 1: SINGLE SCOPED VARIABLE FOR THIS EVENT ONLY ---
          let capturedPricesForThisEvent = null; 

          if (interactive && stepMode) {
            await promptEnter('Press Enter to open the next event link... ', { timeoutMs: 30_000, skipIfNoTty: true });
          }
          ...
          await eventPage.goto(href, {
            ...
            timeout: 60_000,
            referer: rowReferer || listingReferer
          });
          await sleep(2000);
          ...
          if (ticketsHref) {
            ...
            try {
              const clickOutcome = await clickGetTicketsAndWait({ page: eventPage, browser, timeoutMs: 30_000 });
              ...
              if (clickOutcome?.navigated) {
                ...
                if (userAgent && !noUaOverride) await ticketsPage.setUserAgent(userAgent);
                await ticketsPage.setViewport({ width: 1280, height: 800 });
              }

              // --- FIX 2: ATTACH LISTENER TO THE ACTIVE TAB IMMEDIATELY ---
              ticketsPage.on('response', async (response) => {
                const url = response.url();
                if (url.includes('/price') && url.includes('pick_a_seat_2d')) {
                  try {
                    const data = await response.json();
                    console.log('--- Price Data Intercepted! ---');
                    capturedPricesForThisEvent = data.offerPrices.flatMap((offer) => 
                      offer.zonePrices.flatMap((zone) => 
                        zone.priceLevels.map((pl) => ({
              ...
              if (!clickOutcome?.navigated) {
                await ticketsPage.goto(ticketsHref, { waitUntil: 'networkidle2', timeout: 60_000, referer: href });
              }

              await sleep(2000);
              await waitForManualChallengeClear(ticketsPage, { interactive });

              // --- FIX 3: THE "WAIT-AND-RELOAD" SYNC ---
              console.log('Refreshing map to trigger Price API...');
              await ticketsPage.reload({ waitUntil: 'networkidle2' });
              
              // Polling Loop: Wait up to 12 seconds for the variable to be populated
              let waitSecs = 0;
              while (capturedPricesForThisEvent === null && waitSecs < 24) {
                await sleep(500);
                waitSecs++;
              }

              if (interactive) {
                await promptEnter('Review map. Press Enter to save data... ', { skipIfNoTty: true });
              }

              // --- FIX 4: ASSIGN DATA BEFORE TAB CLOSES ---
              details.prices = capturedPricesForThisEvent ? JSON.stringify(capturedPricesForThisEvent) : 'N/A';

              const ticketsHtmlPath = path.join(eventsDir, `${eventId}-tickets.html`);
              await saveHtml({ page: ticketsPage, filePath: ticketsHtmlPath });
              await ticketsPage.screenshot({ path: path.join(eventsDir, `${eventId}-tickets.png`), fullPage: true });

            } finally {
              if (ticketsPage && closeTicketsPage) await ticketsPage.close();
            }
          } else {
            details.prices = 'N/A';
          }

          // --- FIX 5: PUSH COMPLETED OBJECT TO SCRAPED ARRAY ---
          scraped.push(details);

          const base = `${eventId}-${safeFilename(details.name || label)}`;
          await saveHtml({ page: eventPage, filePath: path.join(eventsDir, `${base}.html`) });
          console.log(`Successfully Scraped: ${details.name} | Prices: ${details.prices !== 'N/A'}`);
