...
async function main() {
  ...
  try {
    ...
    if (followAllEvents) {
      ...
      // MANDATORY: Visit the team page first to establish session cookies/handshake
      console.log(`Establishing session at entry point: ${url}`);
      await page.goto(url, { waitUntil: 'networkidle2', timeout: 60_000 });
      
      // Capture the validated URL to use as the Referer for database links
      const listingReferer = page.url();

      // Solve the captcha on the entry page to "unlock" the session
      await waitForManualCaptchaSolve(page, { interactive });
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          // Force the Referer header so AXS thinks you clicked a link on their site
...
await eventPage.goto(visitHref, {
  waitUntil: 'domcontentloaded', // Fast load to catch the security challenge
  timeout: 60_000
});

// Give the security challenge 2 seconds to appear, then solve it
await sleep(2000);
