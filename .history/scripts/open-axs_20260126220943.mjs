...
async function humanizePage(page, { kind }) {
	// “Human-like” pacing and interaction for stability only.
	// This does NOT bypass CAPTCHAs or evade detection.
	try {
		// Small mouse movements
		const moves = randInt(2, 5);
		for (let i = 0; i < moves; i++) {
			await page.mouse.move(randInt(120, 900), randInt(120, 650), { steps: randInt(5, 18) });
			await humanPause(150, 600);
		}
	} catch {
		// ignore
	}
	...
	try {
		// A bit of scrolling to trigger lazy rendering
		const scrolls = kind === 'listing' ? randInt(3, 7) : randInt(1, 4);
	...
	// Give the browser a moment to finish background requests.
	try {
...
async function main() {
	...
	const humanMode = hasFlag('--human') || interactive;
	const linksOnly = hasFlag('--links-only');
	...
	try {
		...
		if (humanMode) await humanizePage(page, { kind: 'landing' });
		...
		if (followAllEvents) {
			...
			if (humanMode) {
				await humanizePage(page, { kind: 'listing' });
			}
			...
			if (linksOnly) {
				const linksPathJson = path.join(artifactsDir, 'event-links.json');
				const linksPathXlsx = path.join(artifactsDir, 'event-links.xlsx');
				await fs.writeFile(linksPathJson, JSON.stringify(links, null, 2), 'utf8');
				await writeXlsx({ rows: links, filePath: linksPathXlsx });
				console.log(`Saved event links JSON: ${linksPathJson}`);
				console.log(`Saved event links XLSX: ${linksPathXlsx}`);
				console.log('Note: --links-only does not open event pages (avoids CAPTCHAs during automation).');
				return;
			}
			...
			try {
				...
				for (let i = 0; i < toVisit.length; i++) {
					...
					if (humanMode) await humanizePage(eventPage, { kind: 'event' });
		...
		if (keepOpen) {
			try {
				await promptEnter('Browser is staying open (--keep-open). Press Enter to close... ');
			} catch {
				// If the user aborts here, we'll let the outer catch handle it.
			}
		}
		await browser.close();
	}
