        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          if (ticketsHref) {
            ...
            try {
              ...
              const waitForPriceResponse = (timeoutMs = 45_000) => {
                return ticketsPage
                  .waitForResponse(
                    (r) => {
                      const u = r.url();
                      return u.includes('/price') && (u.includes('pick_a_seat') || u.includes('pick-a-seat'));
                    },
                    { timeout: timeoutMs }
                  )
                  .catch(() => null);
              ...
              const priceRespPromise = waitForPriceResponse(45_000);
              await ticketsPage.reload({ waitUntil: 'networkidle2' });

              const priceResp = await priceRespPromise;
              if (priceResp) {
                try {
