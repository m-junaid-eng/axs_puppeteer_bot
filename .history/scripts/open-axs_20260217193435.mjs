...
await page.authenticate({
  ...
  try {
    ...
    if (followAllEvents) {
      ...
      // Important: Re-authenticate proxy for the new tab
      await eventPage.authenticate({ username: proxy.user, password: proxy.pass });
      await eventPage.setViewport({ width: 1280, height: 800 });

      for (let i = 0; i < supabaseRows.length; i++) {
        const row = supabaseRows[i];
        const entryUrl = row.list_event_url; // The referer page
        const targetUrl = toAxsEventDetailUrl(row.url); // The event page
        const eventId = String(row.url).match(/\/events\/(\d+)/)?.[1] || row.id;

        console.log(`\n--- [${i + 1}/${supabaseRows.length}] Processing: ${row.name} ---`);

        // STEP 1: Visit Artist/Team List Page (The Handshake)
        console.log(`Step 1: Priming session at List URL: ${entryUrl}`);
        await eventPage.goto(entryUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
        await sleep(3000);
        await waitForManualCaptchaSolve(eventPage, { interactive });

        // STEP 2: Jump to Event Page with Referer spoofing
        console.log(`Step 2: Navigating to Event Detail: ${targetUrl}`);
        await eventPage.setExtraHTTPHeaders({ 'referer': entryUrl });

        // Use window.location.href to look like a real browser click
        await eventPage.evaluate((t) => { window.location.href = t; }, targetUrl);

        try {
          await eventPage.waitForNavigation({ waitUntil: 'networkidle2', timeout: 45000 });
        } catch (e) {
          console.log("Wait timed out, continuing...");
        }

        // STEP 3: Handle Block recovery
        await sleep(2000);
        const isBlocked = await eventPage.evaluate(() => document.body.innerText.includes("We're sorry"));
        if (isBlocked) {
          console.log("Block detected. Attempting cookie-sync reload...");
        ...
        // --- Your existing detail extraction and price interception logic goes here ---
        const details = await extractEventDetails(eventPage, { fallbackUrl: targetUrl, eventId });
        scraped.push(details);

        await saveHtml({ page: eventPage, filePath: path.join(eventsDir, `${eventId}.html`) });
        console.log(`Successfully finished event ${eventId}`);

        await sleep(delayMs); // Cooldown to avoid proxy flagging
      }
