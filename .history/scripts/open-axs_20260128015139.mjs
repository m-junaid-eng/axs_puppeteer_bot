...
async function isLikelyCaptchaPage(page) {
	try {
		...
		const hasCaptchaHints = await page.evaluate(() => {
			...
			// NOTE: We intentionally do NOT treat script tags or CF globals alone as definitive,
			// because many real pages keep those loaded even after a successful solve.
			if (
...
async function waitForManualChallengeClear(page, { interactive, timeoutMs = 10 * 60_000 } = {}) {
	...
	//
	// This does NOT bypass challenges. In interactive mode it simply waits for you
	// to solve the challenge in the visible browser.
...
async function highlightGetTicketsCta(page) {
	try {
		await page.evaluate(() => {
			const anchors = Array.from(document.querySelectorAll('a[href]'));
			const pick = anchors.find((a) => {
				const text = (a.textContent || '').trim().toLowerCase();
				const aria = (a.getAttribute('aria-label') || '').trim().toLowerCase();
				return text.includes('get tickets') || aria.includes('get tickets');
			});
			if (!pick) return;
			pick.scrollIntoView({ block: 'center', inline: 'center' });
			pick.style.outline = '3px solid #ff1744';
...
async function dismissCommonOverlays(page) {
	// Best-effort cookie/banner dismissal. This is not required for normal operation,
	// but can unblock clicks when consent banners cover the CTA.
	try {
		await page.evaluate(() => {
			const needles = [
				'accept',
...
async function clickGetTicketsCta(page) {
	...
	// Returns { clicked, href, tag }.
	try {
		const res = await page.evaluate(() => {
			const needle = 'get tickets';
			const candidates = Array.from(
			...
			// Prefer anchors that obviously go to ticketing.
			const score = (el) => {
				const tag = (el.tagName || '').toLowerCase();
				const href = tag === 'a' ? String(el.getAttribute('href') || '') : '';
				let s = 0;
				if (href) s += 2;
				if (href.includes('tix.axs.com')) s += 5;
				return s;
			};
			...
			if (!best) return { clicked: false, href: null, tag: null };
			...
			const href = tag === 'a' ? best.href : null;
			try {
				best.click();
				return { clicked: true, href, tag };
			} catch {
				return { clicked: false, href, tag };
			}
		});
		return res;
	} catch {
		return { clicked: false, href: null, tag: null };
	}
}

async function clickGetTicketsAndWait({ page, browser, timeoutMs = 25_000 } = {}) {
	// Attempts a real click and waits for either:
	//  - same-tab navigation, or
	//  - a new tab/popup opened by the click
	// Returns { page: ticketsPage, openedNewTab, navigated }
	...
	const popupPromise = browser
		...
		.then(async (t) => {
			...
			return p ? { type: 'popup', page: p } : null;
		})
		.catch(() => null);

	const clickInfo = await clickGetTicketsCta(page);
	if (!clickInfo?.clicked) {
		return { page, openedNewTab: false, navigated: false };
	}

	const winner = await Promise.race([popupPromise, navigationPromise]);
	if (winner?.type === 'popup' && winner.page) {
		try {
			await winner.page.bringToFront();
		} catch {
			// ignore
		}
		return { page: winner.page, openedNewTab: true, navigated: true };
	}

	// Even if navigation didn't fire, the URL can still change via JS.
	const urlChanged = page.url() !== startedUrl;
	return { page, openedNewTab: false, navigated: Boolean(winner || urlChanged) };
}
...
function getArgValue(flag) {
	...
	return val;
						eventPage.__ticketsPage = undefined;
}
...
async function waitForLikelyEventPage(page, { timeoutMs }) {
	...
	while (Date.now() - started < timeoutMs) {
		if (await isLikelyCaptchaPage(page)) return false;
								...
								// Detect a popup tab opened by the manual click (if any).
								const manualPopup = await browser
									.waitForTarget((t) => t.opener() === eventPage.target(), { timeout: 2000 })
									.then((t) => t.page())
									.catch(() => null);
								const ticketsPage = manualPopup || eventPage;
								if (ticketsPage !== eventPage) {
									try {
										await ticketsPage.bringToFront();
									} catch {
										// ignore
									}
								}

								await dismissCommonOverlays(ticketsPage);
								await sleep(800);

								const openedNewTab = ticketsPage !== eventPage;
								const cleared1 = await waitForManualChallengeClear(ticketsPage, {
									interactive,
									timeoutMs: ticketsTimeoutMs
								});
								if (!cleared1 || (await isLikelyCaptchaPage(ticketsPage))) {
									console.log(
										`Tickets page still blocked by CAPTCHA for event ${eventId}; saving blocked artifacts.`
									);
									const blockedHtmlPath = path.join(eventsDir, `${base}-tickets-blocked.html`);
									await saveHtml({ page: ticketsPage, filePath: blockedHtmlPath });
									const blockedShotPath = path.join(eventsDir, `${base}-tickets-blocked.png`);
									await ticketsPage.screenshot({ path: blockedShotPath, fullPage: true });
								} else {
									if (interactive) {
										await promptEnter(
									...
									const cleared2 = await waitForManualChallengeClear(ticketsPage, {
										interactive,
										timeoutMs: ticketsTimeoutMs
									});
									if (!cleared2 || (await isLikelyCaptchaPage(ticketsPage))) {
										console.log(
											`Tickets page re-challenged for event ${eventId}; saving blocked artifacts.`
										);
										const blockedHtmlPath = path.join(eventsDir, `${base}-tickets-blocked.html`);
										await saveHtml({ page: ticketsPage, filePath: blockedHtmlPath });
										const blockedShotPath = path.join(eventsDir, `${base}-tickets-blocked.png`);
										...
										if (stopAfterTickets && interactive) {
											console.log('Stopping after tickets capture (per --stop-after-tickets).');
											break;
										}
									}
								}
		...
		const hasEventHints = await page.evaluate(() => {
								await dismissCommonOverlays(eventPage);
								await sleep(600);
		...
		await sleep(500);
								...
								if (!cleared1 || (await isLikelyCaptchaPage(ticketsPage))) {
									...
									if (!cleared2 || (await isLikelyCaptchaPage(ticketsPage))) {
										...
										// If tickets opened in a new tab, close it to keep sessions tidy.
										if (openedNewTab) {
											try {
												await ticketsPage.close();
											} catch {
...
async function main() {
	...
	const captureTickets = hasFlag('--tickets');
	const manualTickets = hasFlag('--manual-tickets');
	const stopAfterTickets = hasFlag('--stop-after-tickets');
	const ticketsTimeoutMsRaw = getArgValue('--tickets-timeout-ms');
	const ticketsTimeoutMs = ticketsTimeoutMsRaw ? Number(ticketsTimeoutMsRaw) : 180_000;
...
await page.setExtraHTTPHeaders({
  'accept-language': 'en-US,en;q=0.9'
		...
		if (followAllEvents) {
			...
			// Reuse the same tab for events + tickets (reduces re-challenges vs opening new tabs).
			...
			for (let i = 0; i < toVisit.length; i++) {
					...
					if (captureTickets) {
						...
						if (ticketsHref) {
							if (manualTickets && interactive) {
								console.log('Manual tickets mode enabled.');
								console.log('Please click the "Get Tickets" button yourself in the browser.');
								await highlightGetTicketsCta(eventPage);
								await promptEnter(
									'When the tickets/prices page opens (or CAPTCHA shows), press Enter to continue... ',
									{ skipIfNoTty: true }
								);
							} else {
								console.log('Clicking "Get Tickets" button...');
								const clickRes = await clickGetTicketsAndWait({
									page: eventPage,
									browser,
									timeoutMs: 25_000
								});

								let ticketsPage = clickRes.page;
								if (!clickRes.navigated) {
									console.log(`Click did not navigate; falling back to opening: ${ticketsHref}`);
									await eventPage.goto(ticketsHref, {
										waitUntil: 'domcontentloaded',
										timeout: 60_000,
										referer: href
									});
									ticketsPage = eventPage;
								}
								await sleep(1500);

								// Continue below using ticketsPage (may be a popup tab).
								eventPage.__ticketsPage = ticketsPage;
							}

							const ticketsPage = eventPage.__ticketsPage ?? eventPage;
							const openedNewTab = ticketsPage !== eventPage;
							const cleared1 = await waitForManualChallengeClear(ticketsPage, {
								interactive,
								timeoutMs: ticketsTimeoutMs
