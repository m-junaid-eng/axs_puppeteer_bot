...
async function waitForManualCaptchaSolve(page, { interactive }) {
  ...
  // 1. Initial detection: Give Cloudflare time to actually mount the challenge
  let challengeDetected = false;
  for (let i = 0; i < 20; i++) { // Check for 10 seconds
  ...
  console.log('Action: Solve the challenge in the browser window.');
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (currentlyChallenged) {
      clearedStreak = 0; // Reset streak if we see the challenge again
      if (Date.now() - lastLog > 15_000) {
        console.log('Waiting for manual solve...');
        lastLog = Date.now();
      }
      await sleep(1000);
      continue;
    ...
    // 2. The "Streak" Check: Ensure the page stays clear for at least 3 seconds
    // This prevents the script from acting during a temporary redirect/refresh
    clearedStreak++;
    if (clearedStreak < 6) { 
      await sleep(500);
      continue;
    ...
    // 3. Stabilization: Final pause to let session cookies propagate
    console.log('Challenge cleared. Stabilizing session for 3s...');
    await sleep(3000);

    // 4. Verify we are actually on an event page or valid AXS content
    const ok = await waitForLikelyEventPage(page, { timeoutMs: 10000 });
    if (ok) {
      console.log('Success: Reached event content.');
      return;
    }
    
    // If not "OK" and not "Captcha", the page might still be loading
    await sleep(1000);
  }
...
async function main() {
  ...
  ignoreDefaultArgs: ['--enable-automation'], // Crucial for Cloudflare
