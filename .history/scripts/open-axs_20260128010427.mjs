...
async function clickGetTicketsCta(page) {
	...
	// Returns true if we found a candidate and attempted a click.
	try {
		const handle = await page.$x(
			"//a[@href and (contains(translate(normalize-space(string(.)), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'get tickets') or contains(translate(@aria-label, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'get tickets'))]"
		);
		const el = handle?.[0] ?? null;
		if (!el) return false;
		...
		// A small delay makes click timing look more like a human action.
...
async function clickGetTicketsAndWait({ page, browser, timeoutMs = 25_000 } = {}) {
	...
	const clicked = await clickGetTicketsCta(page);
	if (!clicked) {
