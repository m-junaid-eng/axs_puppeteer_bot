...
async function main() {
  ...
  const { browser, page } = await connect({
    ...
    args: [
  '--start-maximized',
  '--disable-blink-features=AutomationControlled' // This is the most important flag
],
    ...
    if (followAllEvents) {
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
         ...
         // Inside the loop, replace your eventPage.goto with this:
const visitHref = useSupabase ? toAxsEventDetailUrl(href) : href;

// Instead of eventPage.goto, try an 'evaluate' navigation. 
// This mimics a real user typing in the address bar better.
await eventPage.evaluate((target) => {
  window.location.href = target;
}, visitHref);

// Since we used window.location, we must manually wait for the load
try {
  await eventPage.waitForNavigation({ waitUntil: 'networkidle2', timeout: 60000 });
} catch (e) {
  console.log("Navigation timeout, but proceeding to check for captcha...");
}

// Set the referer header so AXS thinks you clicked from the team page
await eventPage.setExtraHTTPHeaders({ 'referer': listingReferer });

await eventPage.goto(visitHref, {
  waitUntil: 'domcontentloaded', // Fast load to catch the captcha
  timeout: 60_000
});
await sleep(3000); // Wait for the error to potentially appear

const isBlocked = await eventPage.evaluate(() => {
  return document.body.innerText.includes("We're sorry") || 
         document.title.includes("Access Denied");
});

if (isBlocked) {
  console.log("Blocked detected. Performing session recovery reload...");
  await sleep(2000);
  await eventPage.reload({ waitUntil: 'networkidle2' });
  await sleep(3000);
}
// Give the captcha a moment to load, then solve it
await sleep(2000);
