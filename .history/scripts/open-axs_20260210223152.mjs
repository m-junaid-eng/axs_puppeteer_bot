...
// --- NEW PROXY BRIDGE LOGIC ---
const PROXY_USER = 'watchdog_cdL8M'; 
const PROXY_PASS = 'Watchdog_4990';
const UK_PORT = '8006';

// This creates a local bridge (e.g., http://127.0.0.1:12345)
// It handles the Oxylabs login for you so the browser stays "silent"
const originalProxyUrl = `http://user-${PROXY_USER}:${PROXY_PASS}@isp.oxylabs.io:${UK_PORT}`;
const anonymizedProxyUrl = await proxyChain.anonymizeProxy(originalProxyUrl);

console.log(`Local Bridge Active: ${anonymizedProxyUrl}`);

const { browser, page } = await connect({
    headless: false,
    args: [
        '--start-maximized',
        `--proxy-server=${anonymizedProxyUrl}`, // Chrome thinks it has no proxy password
        '--disable-http2',                     // Prevents multiplexing resets
...
// IMPORTANT: REMOVE or COMMENT OUT your 'await page.authenticate()' line.
// The bridge handles authentication now. If you leave it, it might cause conflicts.
await page.authenticate({
    username: PROXY_USER,
    password: PROXY_PASS
});

console.log(`Connected via Oxylabs UK Proxy on port ${UK_PORT}`);
  try {
