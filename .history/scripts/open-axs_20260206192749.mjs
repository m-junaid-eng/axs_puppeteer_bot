...
const SUPABASE_URL = 'https://lnkaympiinecpfumfvei.supabase.co/';
const SUPABASE_ANON_KEY = 'sb_publishable_StFc8yTai0HtpNV8eVCgHQ_6dmbsNfJ';
...
function toAxsEventDetailUrl(inputUrl) {
  try {
    const u = new URL(String(inputUrl));
    if (u.hostname.endsWith('axs.com') && u.pathname.endsWith('-tickets')) {
...
async function fetchSupabaseAxsLinks({ limit }) {
...
async function main() {
  ...
  const supabaseLimitRaw = getArgValue('--supabase-limit');
  const supabaseLimit = supabaseLimitRaw ? Number(supabaseLimitRaw) : 50;
  ...
  const followAllEvents = hasFlag('--all-events') || useSupabase;
  ...
  // Add these specific flags to bypass the session integrity check
const { browser, page } = await connect({
    headless: false,
    args: [
        '--start-maximized',
        '--disable-blink-features=AutomationControlled', // Hides Puppeteer
        '--disable-web-security',                         // Helps with cross-domain referers
  ...
  try {
    ...
    if (followAllEvents) {
      const scraped = [];
      // This "primes" the session so AXS sees you as a real visitor first
      await page.goto(url, { waitUntil: 'networkidle2', timeout: 60_000 });
      await waitForManualCaptchaSolve(page, { interactive });
      // 1. Visit the team page to generate the security cookie
console.log(`Establishing session at: ${url}`);
await page.goto(url, { waitUntil: 'networkidle2', timeout: 60_000 });

// 2. Wait extra time for background security scripts (DataDome/Cloudflare) to settle
await sleep(5000); 

// 3. Solve the captcha if it appears
await waitForManualCaptchaSolve(page, { interactive });

// 4. Store the current URL as the legitimate Referer
const listingReferer = page.url();
      ...
      let links = [];
      if (useSupabase) {
        const limit = Number.isFinite(supabaseLimit) && supabaseLimit > 0 ? supabaseLimit : 50;
        const rows = await fetchSupabaseAxsLinks({ limit });
        links = rows
          .map((r) => ({ href: r?.url, label: r?.name || r?.url }))
          .filter((x) => typeof x.href === 'string' && x.href.trim().length > 0);
      } else {
        links = await collectSeeEventLinksInAllEventsSection(page);
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          const visitHref = useSupabase ? toAxsEventDetailUrl(href) : href;

// Set the referer to the Team Page for every request
await eventPage.setExtraHTTPHeaders({ 
  'referer': listingReferer,
  'accept-language': 'en-US,en;q=0.9'
});

// Navigate with a slightly longer timeout and specific wait
const visitHref = useSupabase ? toAxsEventDetailUrl(href) : href;

// Instead of a direct 'goto', we tell the browser to navigate internally.
// This preserves the session cookies much better than a direct Puppeteer jump.
await eventPage.evaluate((targetUrl) => {
    window.location.href = targetUrl;
}, visitHref);

// Since we didn't use 'goto', we must manually wait for the load
try {
    await eventPage.waitForNavigation({ waitUntil: 'networkidle2', timeout: 60000 });
} catch (e) {
    console.log("Navigation took too long, but checking page state anyway...");
}

// CRITICAL: Give AXS 3 seconds to validate your cookie before trying to extract data
await sleep(3000); 

// If the "We're sorry" message is STILL there, try a single reload
const isBlocked = await eventPage.evaluate(() => document.body.innerText.includes("We're sorry"));
if (isBlocked) {
  console.log("Detected block. Attempting session recovery reload...");
  await sleep(2000);
  await eventPage.reload({ waitUntil: 'networkidle2' });
}
          await sleep(1500);
