...
async function fetchAxsLinksFromSupabase({ limit }) {
  const supabaseUrl = process.env.SUPABASE_URL ?? 'https://lnkaympiinecpfumfvei.supabase.co/';
  const supabaseKey =
  ...
  if (error) {
    throw new Error(`Select error: ${error.message ?? JSON.stringify(error)}`);
  }
...
async function main() {
  ...
  const url = useSupabase ? 'about:blank' : getArgValue('--url') ?? DEFAULT_URL;
  ...
  try {
    ...
    if (!useSupabase) {
      ...
      await page.goto('about:blank', { waitUntil: 'domcontentloaded', timeout: 60_000 });
    }

    if (saveMainHtml && !useSupabase) {
    ...
    if (dumpFingerprint && !useSupabase) {
    ...
    if (followAllEvents) {
      const scraped = [];
      let listingReferer = 'https://www.axs.com/';

      /** @type {{href: string, label: string}[]} */
      let links = [];

      if (useSupabase) {
        const rows = await fetchAxsLinksFromSupabase({
          limit: Number.isFinite(supabaseLimit) && supabaseLimit > 0 ? supabaseLimit : 50
        });

        links = rows
          .map((r) => ({ href: String(r?.url || ''), label: String(r?.name || r?.url || '') }))
          .filter((x) => x.href && x.href.startsWith('http'));

        if (links.length === 0) {
          console.log('No event URLs returned from Supabase (axs_links.url).');
          return;
        }
      } else {
        listingReferer = page.url();
      ...
      try {
        ...
        // Supabase mode: ensure we have a valid AXS session/cookies before opening deep links.
        if (useSupabase) {
          console.log('Supabase mode: warming up AXS session...');
          await eventPage.goto('https://www.axs.com/', { waitUntil: 'domcontentloaded', timeout: 60_000 });
          await sleep(1500);
          await waitForManualCaptchaSolve(eventPage, { interactive });
          listingReferer = eventPage.url();
        }
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          if (ticketsHref) {
            ...
            try {
              ...
              await ticketsPage.setExtraHTTPHeaders({
                'accept-language': 'en-US,en;q=0.9'
              });
