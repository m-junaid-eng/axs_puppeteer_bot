...
const DEFAULT_SUPABASE_URL = 'https://lnkaympiinecpfumfvei.supabase.co/';
const DEFAULT_SUPABASE_KEY = 'sb_publishable_StFc8yTai0HtpNV8eVCgHQ_6dmbsNfJ';
...
function pickFirstUrlFromRow(row) {
...
async function fetchSupabaseTicketUrls({ supabaseUrl, supabaseKey, table = 'axs_links', limit = 50 }) {
  ...
  if (error) {
    throw new Error(`Supabase select error: ${error.message || String(error)}`);
  }

  const rows = Array.isArray(data) ? data : [];
  const urls = rows.map(pickFirstUrlFromRow).filter(Boolean);
  return { rows, urls };
}
...
async function main() {
  ...
  const supabaseMode = hasFlag('--supabase');
  const supabaseUrl =
    getArgValue('--supabase-url') ??
    process.env.SUPABASE_URL ??
    DEFAULT_SUPABASE_URL;
  const supabaseKey =
    getArgValue('--supabase-key') ??
    process.env.SUPABASE_KEY ??
    DEFAULT_SUPABASE_KEY;
  const supabaseTable = getArgValue('--supabase-table') ?? process.env.SUPABASE_TABLE ?? 'axs_links';
  const supabaseLimit = coerceNumber(getArgValue('--supabase-limit') ?? process.env.SUPABASE_LIMIT, 50);
  ...
  try {
    ...
    if (supabaseMode) {
      console.log('Supabase mode enabled: loading recent ticket URLs...');
      const { rows, urls } = await fetchSupabaseTicketUrls({
        supabaseUrl,
        supabaseKey,
        table: supabaseTable,
        limit: supabaseLimit
      ...
      if (urls.length === 0) {
        console.log(`No usable URLs found in Supabase table "${supabaseTable}" (checked ${rows.length} rows).`);
        return;
      }

      const ticketsDir = path.join(artifactsDir, 'tickets');
      ...
      for (let i = 0; i < urls.length; i++) {
        ...
        console.log(`URL ${i + 1}/${urls.length}: ${inputUrl}`);

        let interceptedPrices = [];

        const attachPriceInterceptor = (p) => {
          p.on('response', async (response) => {
            const resUrl = response.url();
            if (resUrl.includes('/price') && resUrl.includes('pick_a_seat_2d')) {
              try {
                const data = await response.json();
                console.log('--- Price API Intercepted Successfully! ---');
                interceptedPrices = data.offerPrices.flatMap((offer) => {
        ...
        try {
          if (userAgent && !noUaOverride) {
            await entryPage.setUserAgent(userAgent);
          }
          await entryPage.setViewport({ width: 1280, height: 800, deviceScaleFactor: 1 });
          attachPriceInterceptor(entryPage);

          await entryPage.goto(inputUrl, { waitUntil: 'domcontentloaded', timeout: 60_000 });
          await sleep(1500);
          await waitForManualChallengeClear(entryPage, { interactive });

          const ticketsHref = await findGetTicketsHref(entryPage);
          if (ticketsHref) {
            console.log(`Found Get Tickets link. Clicking: ${ticketsHref}`);
            ...
            if (clickOutcome?.navigated) {
              ...
              openedNewTab = Boolean(clickOutcome.openedNewTab);
              if (ticketsPage !== entryPage) {
                attachPriceInterceptor(ticketsPage);
                if (userAgent && !noUaOverride) {
              ...
              console.log('Click did not navigate; opening tickets link directly...');
              ...
              await ticketsPage.setViewport({ width: 1280, height: 800, deviceScaleFactor: 1 });
              await ticketsPage.goto(ticketsHref, { waitUntil: 'domcontentloaded', timeout: 60_000, referer: inputUrl });
              await sleep(1500);
            }
          } else {
            console.log('No Get Tickets link found; treating this URL as the tickets page.');
            ticketsPage = entryPage;
          }

          await waitForManualChallengeClear(ticketsPage, { interactive });

          console.log('Refreshing Seat Map to capture prices...');
          await ticketsPage.reload({ waitUntil: 'domcontentloaded', timeout: 60_000 });
          await sleep(4000);

          const base = `${String(i + 1).padStart(3, '0')}-${safeFilename(inputUrl)}`;
          const htmlPath = path.join(ticketsDir, `${base}.html`);
          const pngPath = path.join(ticketsDir, `${base}.png`);
          await saveHtml({ page: ticketsPage, filePath: htmlPath });
          await ticketsPage.screenshot({ path: pngPath, fullPage: true });

          results.push({
            url: inputUrl,
            prices: interceptedPrices.length > 0 ? interceptedPrices : null,
            savedHtml: htmlPath,
            savedPng: pngPath
          });
        } finally {
          if (ticketsPage && openedNewTab) {
            try {
      ...
      const outJson = path.join(artifactsDir, 'tickets-results.json');
      await fs.writeFile(outJson, JSON.stringify(results, null, 2), 'utf8');
      console.log(`Saved tickets results JSON: ${outJson}`);
      return;
    }
    ...
    if (followAllEvents) {
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          // Reset data for each new event in the loop
          interceptedPrices = [];
          ...
          if (ticketsHref) {
            ...
            try {
              ...
              // --- PRICE INTERCEPTOR ATTACHED TO TICKETS PAGE ---
              ...
              // --- ASSIGN INTERCEPTED DATA TO THE XLSX ROW ---
              details.prices = interceptedPrices.length > 0 ? JSON.stringify(interceptedPrices) : 'N/A';
