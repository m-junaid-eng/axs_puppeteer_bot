...
async function waitForManualCaptchaSolve(page, { interactive }) {
  ...
  console.log('--- Action Required: Solve the Checkbox ---');
  
  // 1. Wait until the CAPTCHA is gone from the DOM
  await page.waitForFunction(() => {
    return !document.querySelector('iframe[src*="challenges.cloudflare.com"]') &&
           !document.body.innerText.includes('Verifying you are human');
  }, { timeout: 0 }); // Wait forever until solved

  // 2. IMPORTANT: DO NOT MOVE THE MOUSE OR NAVIGATE YET
  console.log('Checkmark detected. Entering 7-second "Quiet Zone" for validation...');
  
  // This pause lets Cloudflare send its final telemetry without seeing bot movement
  await sleep(7000); 

  // 3. Confirm we are actually on an AXS page now
