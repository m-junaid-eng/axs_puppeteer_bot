...
async function getCaptchaReasons(page) {
	const reasons = [];
	try {
		const url = String(page.url() || '').toLowerCase();
		if (url.includes('cdn-cgi')) reasons.push('url:cdn-cgi');
		if (url.includes('challenge')) reasons.push('url:challenge');
		if (url.includes('captcha')) reasons.push('url:captcha');
		if (url.includes('arkoselabs')) reasons.push('url:arkoselabs');
	} catch {
		// ignore
	}
	...
	try {
		...
		if (title.includes('just a moment')) reasons.push('title:just-a-moment');
		if (title.includes('checking your browser')) reasons.push('title:checking-your-browser');
		if (title.includes('captcha')) reasons.push('title:captcha');
		if (title.includes('challenge')) reasons.push('title:challenge');
	} catch {
		// ignore
	}
	...
	try {
		const dom = await page.evaluate(() => {
			...
			return {
				textCaptcha: text.includes('captcha') || text.includes('are you human') || text.includes('verify you are'),
				textCloudflare: text.includes('cloudflare') || text.includes('just a moment') || text.includes('checking your browser'),
				hasTurnstileResponse: Boolean(document.querySelector('input[name="cf-turnstile-response"]')),
				hasCfChl: Boolean(document.querySelector('[id^="cf-chl-"]')),
				hasChallengePlatformScript: Boolean(
					document.querySelector('script[src*="/cdn-cgi/challenge-platform/"]')
				),
				hasTurnstileScript: Boolean(
					document.querySelector('script[src*="challenges.cloudflare.com/turnstile"]')
				),
				hasCfGlobals: Boolean(
					// eslint-disable-next-line no-undef
					(typeof window !== 'undefined' && (window._cf_chl_opt || window.__CF$cv$params))
				),
				hasCaptchaIframe: Array.from(document.querySelectorAll('iframe')).some((f) =>
					(f.src || '').toLowerCase().includes('captcha')
				)
			};
		});

		if (dom.textCaptcha) reasons.push('dom:text-captcha');
		if (dom.textCloudflare) reasons.push('dom:text-cloudflare');
		if (dom.hasTurnstileResponse) reasons.push('dom:cf-turnstile-response');
		if (dom.hasCfChl) reasons.push('dom:cf-chl-*');
		if (dom.hasChallengePlatformScript) reasons.push('dom:challenge-platform-script');
		if (dom.hasTurnstileScript) reasons.push('dom:turnstile-script');
		if (dom.hasCfGlobals) reasons.push('dom:cf-globals');
		if (dom.hasCaptchaIframe) reasons.push('dom:captcha-iframe');
	} catch {
		// ignore
	}
...
async function logCaptchaReasons(page, prefix = '') {
	try {
		const reasons = await getCaptchaReasons(page);
		const url = (() => {
			try {
				return page.url();
			} catch {
		...
		const msg = reasons.length ? reasons.join(', ') : '(no specific markers found)';
		console.log(`${prefix}CAPTCHA detected via: ${msg}${url ? ` | url=${url}` : ''}`);
	} catch {
		// ignore
	}
}
...
async function isLikelyCaptchaPage(page) {
	try {
		...
		const hasCaptchaHints = await page.evaluate(() => {
			...
			// IMPORTANT: Do not treat the mere presence of Turnstile scripts as a CAPTCHA.
			// Some non-challenge pages can include these scripts.
			if (
...
async function main() {
	...
	const warmupTickets = hasFlag('--warmup-tickets');
  ...
  defaultViewport: null
	...
	try {
		...
		if (followAllEvents) {
			...
			if (interactive && warmupTickets) {
				console.log('Warmup: opening tix.axs.com once to establish session/cookies (manual solve if prompted)...');
				const warmupPage = await browser.newPage();
				try {
					await configurePageForRun(warmupPage, { userAgent, noUaOverride });
					await warmupPage.goto('https://tix.axs.com/', { waitUntil: 'domcontentloaded', timeout: 60_000 });
					await sleep(1500);
					await waitForManualChallengeClear(warmupPage, { interactive, timeoutMs: ticketsTimeoutMs });
				} catch (e) {
					console.log(`Warmup: failed to load/clear tix.axs.com (${String(e?.message || e)}). Continuing...`);
				} finally {
					try {
