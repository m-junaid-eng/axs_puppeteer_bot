  ...
  try {
    ...
    if (followAllEvents) {
      ...
      eventPage.on('response', async (response) => {
        ...
        // This catches the specific API call AXS makes for seat prices
        if (url.includes('/price') && url.includes('pick_a_seat_2d')) {
          try {
            const data = await response.json();
            console.log('--- Price API Successfully Intercepted ---');

            interceptedPrices = data.offerPrices.flatMap((offer) => {
              return offer.zonePrices.flatMap((zone) => {
                return zone.priceLevels.map((pl) => ({
                  offerName: offer.offerName,
                  label: pl.label, // e.g., PL8
                  price: (pl.prices[0].base / 100).toFixed(2), // Convert cents to dollars
      ...
      try {
        ...
        for (let i = 0; i < toVisit.length; i++) {
          ...
          // Reset prices per event.
          interceptedPrices = [];

          // Use a referer to match normal click navigation behavior.
          // TO THIS:
          await eventPage.goto(href, {
            waitUntil: 'networkidle2', // Crucial: waits for the Price API to finish
            timeout: 60_000,
            referer: listingReferer
          });

          // Add a small sleep to ensure the 'on response' has time to finish processing
          await sleep(3000);
          ...
          // We use JSON.stringify so all prices fit into one Excel cell for that event
          details.prices = interceptedPrices.length > 0 ? JSON.stringify(interceptedPrices) : 'No prices found';
          ...
          if (ticketsHref) {
            ...
            try {
              ...
              // --- PRICE INTERCEPTOR FOR TICKETS PAGE ---
              ticketsPage.on('response', async (response) => {
                const url = response.url();
                if (url.includes('/price') && url.includes('pick_a_seat_2d')) {
                  try {
                    const data = await response.json();
                    console.log('--- Price API Intercepted on Tickets Page ---');
                    interceptedPrices = data.offerPrices.flatMap((offer) => {
                      return offer.zonePrices.flatMap((zone) => {
                        return zone.priceLevels.map((pl) => ({
                          offerName: offer.offerName,
              ...
              if (!clickOutcome?.navigated) {
                await ticketsPage.goto(ticketsHref, {
                  waitUntil: 'networkidle2', // Wait for background API traffic to finish
                  timeout: 60_000,
                  referer: href
                });
              }
              ...
              // --- TRIGGER RELOAD TO CAPTURE DATA AFTER CAPTCHA ---
              console.log('Reloading tickets page to trigger Price API...');
              ...
              // --- SAVE CAPTURED PRICES TO DETAILS ---
              if (interceptedPrices.length > 0) {
                  details.prices = JSON.stringify(interceptedPrices);
              }
