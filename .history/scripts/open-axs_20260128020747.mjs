...
async function clickGetTicketsCta(page) {
	...
	// More robust than only clicking <a>: also supports button/role=button.
		...
		for (const xp of xpaths) {
			...
			for (const el of handles) {
				...
				try {
					...
					// If mouse click fails, try elementHandle.click.
					try {
					...
					if (captureTickets) {
						...
						if (ticketsHref) {
							...
							if (manualTickets && interactive) {
								...
								ticketsPage = clickRes.page;
								openedNewTab = Boolean(clickRes.openedNewTab);
