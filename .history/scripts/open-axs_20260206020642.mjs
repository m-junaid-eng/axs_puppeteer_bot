...
async function fetchAxsLinksFromSupabase({ limit }) {
  ...
  if (!supabaseUrl || !supabaseKey) {
    throw new Error('Missing Supabase config. Set SUPABASE_URL and SUPABASE_KEY.');
  }
...
async function main() {
  ...
  const url = getArgValue('--url') ?? (useSupabase ? 'https://www.axs.com/' : DEFAULT_URL);
  ...
  try {
    ...
    if (followAllEvents) {
      ...
      if (useSupabase) {
        ...
        listingReferer = 'https://www.axs.com/';
        ...
        console.log(`Loaded ${links.length} event URLs from Supabase.`);
      } else {
        await waitForManualCaptchaSolve(page, { interactive });
