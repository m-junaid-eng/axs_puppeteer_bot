...
async function fetchSupabaseEventUrls({ supabaseUrl, supabaseKey, table = 'axs_links', limit = 50 }) {
...
async function main() {
  ...
  const supabaseFlag = hasFlag('--supabase');
  let followAllEvents = hasFlag('--all-events') || supabaseFlag;
  const exportXlsx = followAllEvents && !hasFlag('--no-xlsx');

  // If you pass --supabase, event URLs are loaded from Supabase.
  const useSupabaseForAllEvents = followAllEvents && supabaseFlag;
  const supabaseUrl =
  ...
  try {
    ...
    if (followAllEvents) {
      ...
      let links = [];
      if (useSupabaseForAllEvents) {
        console.log('All-events Supabase mode enabled: loading event URLs from Supabase...');
        const { rows, urls } = await fetchSupabaseEventUrls({
          supabaseUrl,
          supabaseKey,
          table: supabaseTable,
          limit: supabaseLimit
        ...
        links = urls.map((href) => ({ href, label: href }));
      } else {
        await waitForManualCaptchaSolve(page, { interactive });
