...
async function main() {
	...
	try {
		...
		if (keepOpen) {
			console.log('Browser left open (--keep-open). You can continue manually in the browser window.');
			await promptEnter('Press Enter to close the browser and exit... ', { timeoutMs: 0, skipIfNoTty: true });
		}
		await browser.close();
	}
}
